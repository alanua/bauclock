...
from db.dashboard_tokens import dashboard_token_key, parse_dashboard_token_payload
...
class MiniAppBotConfig:
    token: str
    username: str
    role: str
    allow_platform_superadmin: bool = False
...
def _add_miniapp_bot_config(
    configs: list[MiniAppBotConfig],
    seen_tokens: set[str],
    *,
    token: str | None,
    username: str | None,
    role: str,
    allow_platform_superadmin: bool = False,
    admin_usernames: list[str] | tuple[str, ...] | None = None,
) -> None:
    ...
    configs.append(
        MiniAppBotConfig(
            ...
            role=role,
            allow_platform_superadmin=allow_platform_superadmin,
            admin_usernames=tuple(admin_usernames or ()),
        )
    )


def _configured_miniapp_bots() -> list[MiniAppBotConfig]:
    from api.config import settings

    configs: list[MiniAppBotConfig] = []
    seen_tokens: set[str] = set()
    platform_token = settings.PLATFORM_BOT_TOKEN
    _add_miniapp_bot_config(
        configs,
        seen_tokens,
        token=platform_token,
        username=settings.PLATFORM_BOT_USERNAME,
        role="platform",
        allow_platform_superadmin=True,
    )
    _add_miniapp_bot_config(
        configs,
        seen_tokens,
        token=settings.SHARED_CLIENT_BOT_TOKEN,
        username=settings.SHARED_CLIENT_BOT_USERNAME,
        role="shared_client",
    )
    _add_miniapp_bot_config(
        configs,
...
def _validate_miniapp_init_data(
    init_data: str,
    bot_configs: list[MiniAppBotConfig],
) -> tuple[dict[str, Any], MiniAppBotConfig]:
    if not bot_configs:
        raise DashboardAccessError("missing_miniapp_bot_config")
    for bot_config in bot_configs:
        try:
            return (
                validate_telegram_init_data(init_data, bot_token=bot_config.token),
                bot_config,
            )
...
async def get_miniapp_private_worker(
    init_data: str | None,
    db: AsyncSession,
    *,
    bot_token: str | None = None,
    ...
    if not worker or not worker.is_active:
        raise DashboardAccessError("worker_access_denied")
...
async def _get_miniapp_dashboard_worker(
    ...
    *,
    ...
    if username and username in matched_bot.admin_usernames:
